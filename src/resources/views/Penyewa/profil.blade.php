...
<form action="" method="post" style="height:auto;width:50%;display:flex;flex-direction:column;">@csrf
    Username    <br>
    <input type="text" class="form-control" name="username" value="{{ old('username',Session::get('penyewa')->username) }}">
    @error("username")
    <label class="label text-danger">{{$message}}</label>
    @enderror
    <br>
    Email    <br>
    <input type="text" class="form-control"  name="email" value="{{ old('email',Session::get('penyewa')->email) }}">
    @error("email")
    <label class="label text-danger">{{$message}}</label>
    @enderror
    <br>
    Nomor Telepon    <br>
    <input type="text" class="form-control"  name="no_telp" value="{{ old('no_telp',Session::get('penyewa')->no_telp) }}">
    @error("no_telp")
    <label class="label text-danger">{{$message}}</label>
    @enderror
    <br>
    Password    <br>
    <input type="password" class="form-control"  name="password" value="{{ old('password',Session::get('penyewa')->password) }}">
    @error("password")
    <label class="label text-danger">{{$message}}</label>
    @enderror
    <br>
